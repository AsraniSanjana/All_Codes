...
FileInputStream("macroPgm_input.txt")));
...
while(st.hasMoreTokens()) {
String x = st.nextToken();
if((index = x.indexOf("=")) != -1) { x = x.substring(0, index); }
l.add(x); }
ala.add(l);
...
PrintWriter out = new PrintWriter(new FileOutputStream("out_ala_pass"
+ pass + ".txt"), true);
for(List l : ala) {
System.out.println(l);
out.println(l); } }
static void showMnt() throws Exception {
PrintWriter out = new PrintWriter(new FileOutputStream("out_mnt.txt"),
true);
for(MntTuple l : mnt) {
System.out.println(l);
out.println(l); } }
static void showMdt() throws Exception {
PrintWriter out = new PrintWriter(new FileOutputStream("out_mdt.txt"),
true);
for(String l : mdt) {
System.out.println(l);
out.println(l); } }
...
MntTuple x = null;
for(MntTuple m : mnt) {
if(m.name.equalsIgnoreCase(token)) { x = m;
break; } }
if(x != null) {
mdtp = x.index;
...
try { l.set(ctr++, token.substring(index+1,
token.length()));
} catch(Exception e) {} } } }
ala.set(ala_no, l);
return l; }
