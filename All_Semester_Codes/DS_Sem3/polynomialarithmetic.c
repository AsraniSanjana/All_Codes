...
printf("Multiplied polynomial before is : ");
display(start3);
//kumkum ma'am customization
//will add the terms having same exponents hence simplifying
struct node *temp;
temp = start3;
struct node *newhead;
newhead = (struct node *)malloc(sizeof(struct node));
newhead->coef=temp->coef;
newhead->expo=temp->expo;
newhead->link=NULL;
temp = start3->link;
struct node *newtemp;
...
newtemp = newhead;
while(temp != NULL) {
if(temp->expo == newtemp->expo) {
newtemp->coef = newtemp->coef + temp->coef;
} else {
...
printf("Multiplied polynomial after is : ");
display(newhead);
}
