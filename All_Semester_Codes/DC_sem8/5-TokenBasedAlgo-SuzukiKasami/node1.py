...
class Node(suzuki_kazami_pb2_grpc.SuzukiKazamiStub):
    def __init__(self):
        self.queue_data = []
        self.servers = ['localhost:50052','localhost:50053']
        self.has_token = True
    ...
    def ReleaseCriticalSection(self,request,context):
        print(f'Critical Section Released by Node {request.site_id}')
        ...
        if len(queue) == 0:
            ...
            self.enter_Critical_Section(site_id=queue[0],sequence_number=1,in_critical_section=False,has_token=True,server_address=server_address)
            self.queue_data.pop()
            for server in self.servers:
